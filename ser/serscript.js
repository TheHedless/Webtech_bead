const API_BASE = 'http://localhost:9090';
        ...
        async function deleteById() {
            ...
            const token = localStorage.getItem('token');
            if (!token) {
                showStatus('apiStatus', 'Please login first (must be ADMIN)', 'error');
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    showStatus('apiStatus', `Deleted item ${id}`, 'success');
                    // some backends return no body for delete
                    showResponse({ message: `Deleted ${id}` });
                } else if (response.status === 403) {
                    showStatus('apiStatus', 'Access denied - you need ADMIN role', 'error');
                    showResponse({ error: 'Access denied' });
                } else {
